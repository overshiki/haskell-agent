-- | HTTP client for the Kimi Responses endpoint.
module Agent.Kimi.Client
    ( StreamEventCallback
    , createResponse
    , createResponseWith
    , createResponseWithEvents
    , createResponseWithEventsPolicy
    , retryTransientKimiResultWithPolicy
    , kimiProviderConfig
    ) where
...
import Agent.Kimi.Error (classifyFailure)
import Agent.Kimi.Options
import Agent.Kimi.Request (buildRequest)
import Agent.Kimi.Stream (streamAssemblyConfig)
...
createResponseWithEventsPolicy policy options credential request onEvent
    | credential.provider /= KimiProvider = pure $ Left $ ProviderError ApiErrorType
        "agent-kimi requires a Kimi credential"
        Nothing
    | otherwise =
        createResponseWithProviderPolicy
            policy
            (kimiProviderConfig options credential)
            request
            (Just onEvent)
...
retryTransientKimiResultWithPolicy = retryTransientResultWithPolicy
...
kimiProviderConfig
    ...
    -> Credential
    -> ProviderClientConfig
kimiProviderConfig options credential = ProviderClientConfig
    { providerExceptionPrefix = "Kimi request failed"
