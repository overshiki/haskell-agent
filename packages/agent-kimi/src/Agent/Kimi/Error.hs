-- | Kimi-specific normalization of HTTP and streaming failures.
module Agent.Kimi.Error
    ( classifyFailure
    , classifyStreamError
    ) where
...
-- | Classify a non-success response from Kimi.
--
-- Kimi returns plain OpenAI-shaped error envelopes, so the decoded error
...
classifyStreamError streamError
    ...
    | otherwise = ConnectionError
        ("Kimi stream error: " <> streamError.message)
