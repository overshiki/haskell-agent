module Agent.Kimi.RequestSpec (spec) where

import Agent.Error (ApiError(..))
import Agent.Json (rawJsonFromEncoding)
import Agent.Responses.Types
import Agent.Kimi.Options
import Agent.Kimi.Request
...
spec = do
    describe "mapModel" do
        it "uses the configured default for empty models and passes slugs through" do
            let options = defaultClientOptions { defaultModel = "kimi-k2.7-code" }
            mapModel options "" `shouldBe` "kimi-k2.7-code"
            mapModel options "   " `shouldBe` "kimi-k2.7-code"
            mapModel options "kimi-k3" `shouldBe` "kimi-k3"
    ...
    describe "buildRequest" do
        it "forces a stateless streaming Responses request" do
            ...
            KeyMap.lookup "model" object `shouldBe` Just (Aeson.String "kimi-k3")
        ...
        it "drops tool_choice none, which Kimi rejects" do
        ...
        it "passes other tool_choice values through" do
            let request :: ResponseCreateParams
                request = sampleRequest
                    { toolChoice = Just (ToolChoiceMode ToolChoiceAuto)
                    }
            object <- expectObject (requestValue defaultClientOptions request)
            KeyMap.lookup "tool_choice" object
                `shouldBe` Just (Aeson.String "auto")
    ...
    describe "SSE assembly" do
        it "decodes typed event constructors and builds the merged final response" do
            let sse = Text.intercalate ""
                    ...
                    , sseBlock "response.completed"
                        "{\"type\":\"response.completed\",\"response\":{\"id\":\"resp-1\",\"created_at\":0,\"model\":\"kimi-k3\",\"status\":\"completed\",\"output\":[],\"usage\":{\"input_tokens\":10,\"output_tokens\":5,\"total_tokens\":15}}}"
                    ]
        ...
        it "maps response.failed and missing completion to transport-level errors" do
            failedEvents <- expectRight $ parseSseEvents $ sseBlock "response.failed"
                "{\"type\":\"response.failed\",\"response\":{\"id\":\"resp-f\",\"created_at\":0,\"model\":\"kimi-k3\",\"status\":\"failed\",\"incomplete_details\":{\"reason\":\"overloaded\"}}}"
