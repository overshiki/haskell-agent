-- | Functional tests for the Kimi client against an in-process HTTP mock.
module Agent.Kimi.ClientSpec (spec) where

import Agent.Error (ApiError(..), ErrorType(..))
import Agent.Kimi.Client
...
spec = do
    describe "createResponseWith" do
        it "POSTs the mapped request with Kimi headers and parses the SSE response" do
            recorded <- newIORef []
            ...
            withMockKimi recorded handler \options -> do
                result <- createResponseWith options
                    (kimiCredential "token-a")
                    (helloRequest "hi")
            ...
            requestModel request `shouldBe` Just "kimi-k3"
        ...
        it "rejects a non-Kimi credential before contacting the host" do
            recorded <- newIORef []
            ...
            withMockKimi recorded handler \options -> do
                ...
                case result of
                    Left (ProviderError ApiErrorType message _) ->
                        message `shouldSatisfy` Text.isInfixOf "Kimi"
                    other -> expectationFailure
                        ("expected Kimi credential error, got " <> show other)
    ...
    describe "retry boundaries" do
        ...
        it "does not retry a transient failure after a callback" do
            ...
            result <- retryTransientKimiResultWithPolicy
                (constantDelay 0 <> limitRetries 3)
                request
        ...
        it "classifies a Kimi 401 envelope as authentication rejected" do
            recorded <- newIORef []
...
withMockKimi
    ...
    -> (ClientOptions -> IO a)
    -> IO a
withMockKimi recorded handler action =
...
kimiCredential :: Text -> Credential
kimiCredential token = Credential
    { accessToken = token
    , accountId = ""
    , leaseId = Nothing
